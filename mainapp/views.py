...
def addtofavorite(request, id):
    ...
    if prod.favorite.filter(id=request.user.id).exists():
        prod.favorite.remove(request.user)
    else:
        prod.favorite.add(request.user)
    return HttpResponseRedirect('/')


def prouct_favorite(request):
    user = request.user
    favorite_product = user.favorite.all()
    context = {
